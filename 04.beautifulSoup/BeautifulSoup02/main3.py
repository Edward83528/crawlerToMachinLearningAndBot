...
def parseLtnNews(uri):
    ...
    for ul_soup in soup.findAll('ul',attrs={"id":"newslistul"}):
        for span_soup in ul_soup.findAll('span'):
            ...
            items.append({"uri":uri,"postdate":postdate,"updatetime":datetime.datetime.now().strftime('%Y-%m-%d')})
            # TO DO
            #print({"uri":uri,"postdate":postdate,"updatetime":datetime.datetime.now().strftime('%Y-%m-%d')})
